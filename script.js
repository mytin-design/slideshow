...
function displayTiles(y) {
    ...
    for(x = 0; x < inds.length;x++) {
        inds[x].className = inds[x].className.replace("active", ""); //loop through all the indicators, and for each, replace the className with "active";
        //this is used by CSS to show current indicator;
    }

    tiles[slideCounter-1].style.display = "block";
    inds[dlideCounter-1].className += "active";
}